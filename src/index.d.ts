...
export type Replica<C extends keyof Replicas = keyof Replicas> =
  ReplicaInstance<
    Replicas[C]["Data"],
    Replicas[C]["Tags"],
    Replicas[C]["WriteLib"]
...
> {
   ...
   */
  Set<P extends LiteralPath<D>>(path: P, value: GetPathValue<D, P>): void;
  /**
   ...
   * ```
   */
  SetValues<P extends LiteralPath<D>>(
    path: P,
    values: GetPathValue<D, P> extends object
      ? Partial<GetPathValue<D, P>>
      : never
  ): void;
  /**
   * Performs `table.insert(t, value)` where `t` is a numeric sequential array `table` located in `path`.
   */
  TableInsert<P extends LiteralPath<D>>(
    path: P,
    value: GetPathValue<D, P> extends Array<infer T> ? T : never,
    index?: number
  ): number;
  /**
   * Performs `table.remove(t, index)` where `t` is a numeric sequential array `table` located in `path`.
   */
  TableRemove<P extends LiteralPath<D>>(
    path: P,
    index: number
  ): GetPathValue<D, P> extends Array<infer T> ? T : never;
   ...
   */
  OnSet<P extends LiteralPath<D>>(
    path: P,
    listener: (
      newValue: GetPathValue<D, P>,
      oldValue: GetPathValue<D, P>
    ) => void
  ): RBXScriptConnection;
  /**
    ...
    listener: (
      ...args: A extends "Set"
        ? [
            action: "Set",
            path: LiteralPath<D>,
            value: GetPathValue<D, LiteralPath<D>>,
            oldValue: GetPathValue<D, LiteralPath<D>>
          ]
        : A extends "SetValues"
        ? [
            action: "SetValues",
            path: LiteralPath<D>,
            values: GetPathValue<D, LiteralPath<D>> extends object
              ? Partial<GetPathValue<D, LiteralPath<D>>>
              : never
          ]
        ...
        ? [
            action: "TableInsert",
            path: LiteralPath<D>,
            value: GetPathValue<D, LiteralPath<D>> extends Array<infer T>
              ? T
              : never,
            index: number
          ]
